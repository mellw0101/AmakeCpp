...
void
link_binary(const vector<string> &obj_vec, const vector<string> &strVec = {})
{
...
void
do_link(const vector<string> &strVec)
{
    std::vector<string> in_files;
    Ulong               n;
    compile_entry      *files = files_in_dir(OBJ_DIR.c_str(), &n);
    for (Ulong i = 0; i < n; ++i)
    {
        ...
        else if (e->type == 4)
        {
            ...
            Ulong          sn;
            compile_entry *sub_files = files_in_dir(string(OBJ_DIR + "/" + e->file).c_str(), &sn);
            for (Ulong si = 0; si < sn; ++si)
            {
                ...
                if (se->type == 8)
                {
                    ...
                    char input[PATH_MAX];
                    snprintf(
                        input, PATH_MAX, "%s/%s/%s%s", OBJ_DIR.c_str(), e->file, se->name, se->ext);
                    in_files.push_back(input);
                }
            }
        }
    ...
    if (installBin)
    {
        try
        {
            ...
            printC(
                to_string(FileSys::fileSize("/usr/bin/" + projectName)) + " Bytes", ESC_CODE_GRAY);
        }
